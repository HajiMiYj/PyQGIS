...
> 说明：
> 1. 前 26 行由 [§6.2](#62-运行) 的 20 个 CLI 套件覆盖，全部通过；最后两行（自定义投影、剖面树/网格移动）是**直接运行**的检查模块（`python-qgis-ltr.bat tests/src/python/xxx.py`），不在这 20 个标志里，同样已验证通过（exit 0、`errors: []`）。
